...
export async function POST(request, { params }) {
  ...
  if (path === 'waitlist') {
    try {
      ...
      if (existingData) {
        return NextResponse.json({ error: 'Tu es déjà inscrit(e) sur la liste ! 🎉' }, { status: 409 })
      }
      ...
      try {
        ...
        await resend.emails.send({
          from: `Sekura 🛡️ <${fromEmail}>`,
          to: [email],
          subject: '🎉 Tu es sur la whitelist Sekura !',
          html: getConfirmationEmail(email, totalCount)
        })
      } catch (emailError) {
